    ...
    mysql.fetchData(function(err,results){
        ...
        else
        {
            ...
            if(results.length > 0){
                ...
                var selectUser="SELECT  * from users where email='"+reqEmail+"'";
                console.log(selectUser);

                mysql.fetchData(function(err,resultnew){
                    if(err){
                        throw (err);
                    }
                    else
                    {
                        console.log("innerquery "+resultnew);
                        if(resultnew.length>0){
                            userdetails.username = resultnew[0].username;
                            userdetails.email = resultnew[0].email;
                        }
                        console.log(resultnew);
                        console.log("last login inserted....")
                        res.send({"status":201, "email" :userdetails.username , "username":  userdetails.email });
...
router.post('/signup', function (req, res) {
  ...
  //  console.log("password: "+reqPassword.passwordHash);
    ...
    var insertUser="insert into users (username, password, email) values ( '"+reqUsername
